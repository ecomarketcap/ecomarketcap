...
export default function CoinMarkets(props) {
  ...
  return (
    <section className='row'>
      <Table
        className='table container p-0 m-0 table-light table-bordered'
        style={colorStyle}
      >
        <thead>
          <tr>
            <Th style={colorStyle}>Exchange</Th>
            <Th style={colorStyle}>Pair</Th>
            <Th style={colorStyle}>Price</Th>
            <Th style={colorStyle}>Confidence</Th>
            <Th style={colorStyle}>Volume (24h)</Th>
            <Th style={colorStyle}>Category</Th>
          </tr>
        </thead>
        <tbody>
          {props.coinMarkets.map((data, index) => {
            return (
              <tr key={index}>
                <TdName>
                  <a
                    href={data.market_url ? data.market_url : ''}
                    style={colorStyle}
                  >
                    {data.exchange_name}
                  </a>
