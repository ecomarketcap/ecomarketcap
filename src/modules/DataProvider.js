...
function buildPricesSetPaprikaUrl(coinId, days) {
  ...
  if (days > 0) {
    url =
      ...
      '?' +
      PAPRIKA_PARAM_START +
      Time.getPastDateByDayInverse(days) +
      '&' +
      PAPRIKA_PARAM_INTERVAL +
      '12h';
  } /*else {
...
export var DataProvider = {
  ...
  getCoinsPriceSetGecko: async (id, devise) => {
    const response = await axios
      .get(buildPricesSetGeckoUrl(id, devise, 7))
      .then((resp) => {
        return resp.data;
      })
      .catch((err) => {
